...
def handle_data(context, data):
    if not context.initialized:
        ...
        context.holding_book_shares = pd.DataFrame(0,columns=context.stocks+['self'],index= context.stocks)
    #print(context.holding_book_shares)
...
def trade(context, data):
    ...
    s,qualified,hedging_book = get_s_score(returns,modified_m=False)
    for sym in s.index:#context.stocks:
        sj = s.loc[sym]
        hedging = hedging_book.loc[:,sym]
        #print(sj)
        ...
        if current_holding_sym == 0:
            if (qualified.loc[sym]) and (sj < -context.bo):
                ...
                for sym2 in hedging.index:
                    orderid_hedge_sym2 = order_percent(sym2,(-1.0)*context.leverage*hedging.loc[sym2])
                    if not sym2 in context.order_hist:
                        context.order_hist[sym2] = {}
                    context.order_hist[sym2][sym] = orderid_hedge_sym2
            ...
            if (((current_holding_sym> 0) and sj>-context.bc) or ((current_holding_sym< 0) and sj<context.sc)):
                ...
                context.order_hist[sym]['self'] = orderid_self
                for sym2 in hedging.index:
                    orderid_hedge_sym2 = order(sym2,context.holding_book_shares.loc[sym2,sym]*(-1))
                    if not sym2 in context.order_hist:
                        context.order_hist[sym2] = {}
                    context.order_hist[sym2][sym] = orderid_hedge_sym2
...
def get_s_score(returns,m=15,modified_m=False,drift_adjusted=False):
    mean_return = returns.mean(axis=0)
    std_return = returns.std(axis=0)
    normalized_returns = (returns-mean_return)/std_return
    Y = normalized_returns.values
    cor_matrix = np.dot(Y.T,Y)/(Y.shape[0])
    cor_matrix = cor_matrix +1e-9*np.eye(cor_matrix.shape[0])
    s,v = np.linalg.eigh(cor_matrix)
    order = np.argsort(s)[::-1]
    s = s[order]
    v = v[:,order]
    lmbda = s[0:m]
    Q = np.divide(v[:,0:m],std_return.values[:,np.newaxis])
    Q = np.divide(Q,np.sum(Q,axis=0))
    F = np.dot(returns,Q)
    ...
    betas = betas[1:,:]
    hedging_book = np.dot(Q,betas)
    hedging_book = pd.DataFrame(hedging_book,columns=returns.columns,index=returns.columns)
    b = OU_fits.loc['b']
    qualified = (b<0.9672) * (b>0)   
    return s_score,qualified,hedging_book
